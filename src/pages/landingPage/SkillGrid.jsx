          ...
          {gridData.map((row, rowIndex) =>
            row.map((cell, colIndex) => {
              ...
              const imageObj = images.find(
                (img) => img.i === rowIndex && img.j === colIndex
              );

              // Build border classes based on cell's true/false edges
              const borders = [];
              if (cell.top) borders.push("border-t-2 border-[#aaa] dark:border-[#444]");
              if (cell.right) borders.push("border-r-2 border-[#aaa] dark:border-[#444]");
              if (cell.bottom) borders.push("border-b-2 border-[#aaa] dark:border-[#444]");
              if (cell.left) borders.push("border-l-2 border-[#aaa] dark:border-[#444]");
              ...
              return (
                <motion.div
                  ...
                  transition={{
                    delay: (rowIndex * 12 + colIndex) * 0.02,
                    duration: 0.4,
                    ease: "easeInOut",
                  }}
                >
                  ...
                  {imageObj && (
                    <motion.img
                      src={`${import.meta.env.BASE_URL}${imageObj.imageLink}`}
                      alt="Skill Icon"
                      className="w-16 h-16 object-contain grayscale-100 hover:grayscale-0 cursor-pointer"
                      whileHover={{
                        scale: 1.1,
                        rotate: 5,
